...
class Release(object):
    ...
    def packZip(self):
        ...
        if os.path.exists('../../mod_mods_gui/release/i18n'):
            subprocess.call('powershell robocopy %s %s %s /COPYALL' % (os.path.realpath(
                '../../mod_mods_gui/release/i18n'), os.path.join(self.modsPath, 'configs', 'mods_gui', 'i18n'), '*.json'))
            subprocess.call('powershell robocopy %s %s %s /COPYALL' % (os.path.realpath(
                '../../mod_mods_gui/release/i18n'), os.path.join(self.modsPath, 'configs', 'mods_gui', 'i18n'), '*.html'))
